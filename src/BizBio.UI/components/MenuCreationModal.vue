  ...
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        <!-- Modal Content -->
        <div
          class="relative bg-md-surface rounded-2xl shadow-xl w-full max-w-7xl my-8 animate-slide-up"
          @click.stop
        >
          <!-- Close Button -->
          <button
            @click="handleClose"
            class="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-md-surface-container hover:bg-md-surface-container-high text-md-on-surface transition-colors"
          >
...
// Reset menu creation when modal is closed
watch(() => props.show, (newVal) => {
  if (newVal) {
    // Modal opened - check subscription
