...
const ICON_SETS = {
    light: {
        ...
        signal: { style: "fa-brands", icon: "fa-signal-messenger" } // may vary by FA kit; see note below
    },
    dark: {}, // inherits from light by default below
    mono: {} // mono uses same icons but you can unify colors
};
...
for (const k of Object.keys(DEFAULT_TITLE)) {
 ...
 *  Public API
 *  ----------------------*/
export function resolveLinks(inputs, opts = {}) {
...
export function resolveOne(url, input = {}, opts = {}) {
    const theme = (opts.theme ?? "light");
    const preferCssVars = opts.preferCssVars ?? true;
    ...
    const fa = {
        style: override?.fa?.style || baseFa.style,
 ...
 *  Detection helpers
 *  ----------------------*/
function detectKindAndHref(url) {
    console.log("Detecting kind and href for URL:", url);
    const u = url.trim();
...
function hostToKind(host, lowerUrl) {
...
function buildAriaLabel(kind, title, href) {
...
function findOverride(url, overrides) {
    if (!overrides)
        return null;
 ...
 */
export function buildSocialCssVariables() {
    const kinds = Object.keys(DEFAULT_TITLE);
    const lines = [];
    lines.push(":root {");
 ...
 */
export function listIconsForTheme(theme = "light") {
    const set = ICON_SETS[theme] ?? ICON_SETS.light;
    const unique = new Map();
    for (const k of Object.keys(DEFAULT_TITLE)) {
        const fa = set[k];
        if (fa)
            unique.set(`${fa.style}|${fa.icon}`, fa);
    }
